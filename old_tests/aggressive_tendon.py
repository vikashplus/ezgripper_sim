...
try:
    ...
    print("🎯 AGGRESSIVE tendon position control")
    print("👀 Watch fingers close with aggressive tendon shortening")
except Exception as e:
    print(f"❌ Could not launch viewer: {e}")
    exit(1)

print("\n" + "=" * 60)
print("AGGRESSIVE TENDON CONTROL - FORCE FINGERS TO TOUCH")
print("=" * 60)
...
print("Applying AGGRESSIVE tendon position to force fingers to touch...")

# Find tendon actuators
f1_actuator = 0  # gripper_actuator_f1
f2_actuator = 1  # gripper_actuator_f2

# Simulation: aggressive tendon control
step = 0
max_steps = 1500

while viewer.is_running() and step < max_steps:
    # Calculate aggressive tendon position (much shorter than normal)
    if step < 400:
        # Phase 1: Aggressively close
        progress = step / 400.0
        # Go much shorter than normal range - force fingers together
        target_length = 0.168 - progress * 0.060  # Very aggressive!
    elif step < 1000:
        # Phase 2: Hold aggressive position
        target_length = 0.108  # Much shorter than minimum!
    else:
        # Phase 3: Release to normal
        progress = (step - 1000) / 500.0
        target_length = 0.108 + progress * 0.050
    ...
    if step % 100 == 0:
        ...
        print(f"  Target tendon length: {target_length:.6f}m (AGGRESSIVE!)")
        ...
        if finger_contacts > 0:
            ...
            print(f"  ⚠️  Fingers still not touching - trying harder!")
    
    # Sync viewer
    viewer.sync()
    step += 1

print("\n🎬 Aggressive tendon control complete!")
print("=" * 60)
